...
const Book: React.FC<BookProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-xl bg-white z-[210] shadow-2xl transition-transform duration-500 ease-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-full flex flex-col p-8 md:p-12 overflow-y-auto">
          {/* Header */}
          <div className="flex justify-between items-center mb-12">
            <h2 className="text-4xl font-extrabold text-[#1a1a1a] font-serif">
              Book tid
            </h2>
            <button 
              onClick={onClose}
          ...
          <form className="space-y-6 flex-1">
            ...
            <div className="flex items-center gap-4 py-4 px-4">
              <input type="checkbox" id="terms-book" className="w-6 h-6 accent-[#d4a373] cursor-pointer" />
              <label htmlFor="terms-book" className="text-sm text-gray-500 cursor-pointer font-medium leading-tight">
